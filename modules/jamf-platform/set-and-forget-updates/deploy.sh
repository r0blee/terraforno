...
#  Set and Forget OS Updates — deploy.sh
#  Provider: Jamf-Concepts/jamfplatform
# ─────────────────────────────────────────

MODULE="jamf-platform/set-and-forget-updates"
# GITHUB_REPO is set centrally in lib/mode.sh
...
echo ""
echo -e "${BLD}  ── Set and Forget OS Updates ──${RST}"
echo ""
...
if [ "$_apply_exit" -ne 0 ]; then
    # Check if this is a known provider inconsistency bug (resource still created)
...
echo ""
echo -e "${GRN}  ✔ Set and Forget OS Updates deployed successfully.${RST}"
